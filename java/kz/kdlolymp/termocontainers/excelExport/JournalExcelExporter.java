...
public class JournalExcelExporter {
    ...
    private void writeTitleLine(){
        ...
        createCell(row, 4, "Отправил", style);
        createCell(row, 5, "Запись отправителя", style);
        createCell(row, 6, "Сумма оплаты", style);
        createCell(row, 7, "Получатель", style);
        createCell(row, 8, "Дата приемки", style);
        createCell(row, 9, "Получил", style);
        createCell(row, 10, "Запись получателя", style);
    ...
    private void writeDataLines() {
        ...
        for (ContainerNote note : notes) {
            ...
            createCell(row, 4, note.getOutUser().getUserFirstname() + " " + note.getOutUser().getUserSurname(), style);
            createCell(row, 5, note.getSendNote(), style);
            createCell(row, 6, note.getSendPay() + " тенге", style);
            createCell(row, 7, note.getToDepartment().getBranch().getBranchName() + ", " + note.getToDepartment().getDepartmentName(), style);
            if(note.getArriveTime()!=null){
                createCell(row, 8, note.getArriveTime().format(formatter), style);
                createCell(row, 9, note.getToUser().getUserFirstname() + " " + note.getToUser().getUserSurname(), style);
                createCell(row, 10, note.getArriveNote(), style);
            ...
            if(note.getBetweenPoints()!=null){
                for(BetweenPoint point: note.getBetweenPoints()){
                    ...
                    createCell(row, 5, "промежуточный объект: ", style);
                    createCell(row, 7, point.getDepartment().getDepartmentName() + ", " + point.getDepartment().getBranch().getBranchName(), style);
                    createCell(row, 8, point.getPassTime().format(formatter), style);
                    createCell(row, 9, point.getPassUser().getUserFirstname() + " " + point.getPassUser().getUserSurname(), style);
                    createCell(row, 10, point.getPointNote(), style);
                    rowNumber++;
                }
            }
        }
